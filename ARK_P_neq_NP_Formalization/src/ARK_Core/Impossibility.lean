...
-- UPDATED: We interpret P=NP to imply a *standard* complexity class (e.g. k <= 99),
-- or at least that the polynomial degree is small relative to the dimension n=1000.
...
theorem p_neq_np_proven : (n E > 1000) → ¬ Hypothesis_PolyGap E := by
  ...
  have h_calc_fact : ¬ ((1 : ℝ) / (n E ^ k : ℝ) ≤ Real.exp (-n E)) := by
    -- e^-n < 1/n^k is equivalent to n^k < e^n
    intro h_false
    ...
    have h_inv : Real.exp (-n E) < (1 : ℝ) / (n E ^ k : ℝ) := by
      ...
      · apply Real.rpow_pos_of_pos
        exact lt_of_le_of_lt (by norm_num) h_dim

    -- Contradiction with h_false (which says 1/n^k ≤ e^-n)
    linarith
